...
print(f'# training dps so far: {len(train_data)}')
print(f'# testing dps so far: {len(test_data)}')
# Specify data
init_train_comments = [item[0] for item in train_data]
train_debt_codes = [item[1] for item in train_data]
train_repaid_codes = [item[2] for item in train_data]
init_test_comments = [item[0] for item in test_data]
...
train_localised_no_comment, train_comments = pair_patterns.extract_patterns(train_debt_codes, train_repaid_codes, init_train_comments)
test_localised_no_comment, test_comments = pair_patterns.extract_patterns(test_debt_codes, test_repaid_codes, init_test_comments)
print(f'{len(init_train_comments)-len(train_localised_no_comment)} training dps did not pass the localisation procedure')
print(f'New # training dps: {len(train_localised_no_comment)}')
