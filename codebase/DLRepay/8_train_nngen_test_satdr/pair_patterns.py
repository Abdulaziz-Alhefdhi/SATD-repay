...
def extract_patterns(debt, repaid, comments):
    # Tokenise (types & values)
    debt_tokenised, repaid_tokenised, passing_comments = [], [], []
    for dt, rp, c in tqdm(zip(debt, repaid, comments), total=len(debt)):
        try:
    ...
    return localised_data, passing_comments
