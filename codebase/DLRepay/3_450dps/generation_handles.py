...
def generate_fixes(buggy_tokenised, fixed_tokenised, cand_num, enc_dec, test_buggy_inputs, max_fixed_len_test, token_int_map, int_token_map, d4j_localised, vocab_size):
    if cand_num == 1:
        ...
        for i, (buggy, fixed, gnrtd) in enumerate(zip(test_buggy_tokenised, test_fixed_tokenised, generated_codes)):
            print('=============')
            print('=============')
            print('SATD code:  ', ' '.join(buggy))
            print('---')
            print('Repaid code:', ' '.join(fixed[1:-1]))
            print('Generation: ', ' '.join(gnrtd[1:-1]))
