...
# Combine
train_localised = [(x[0], x[1], x[2], x[3], x[4]) for x in train_localised_no_comment]
test_localised = [(x[0], x[1], x[2], x[3], x[4]) for x in test_localised_no_comment]

# Don't take too-long codes
train_trimmed = [x for x in train_localised if len(x[1]) <= max_code_length and len(x[3]) <= max_code_length]
test_trimmed = [x for x in test_localised if len(x[1]) <= max_code_length and len(x[3]) <= max_code_length]

# Remove duplicates in training set
train_hashable_trimmed = [(tuple(x[0].items()), tuple(x[1]), tuple(x[2]), tuple(x[3]), tuple(x[4])) for x in train_trimmed]
train_no_dups = sorted(set(train_hashable_trimmed))

# Back to non-hashable
train_clean_localised = [(dict(x[0]), list(x[1]), list(x[2]), list(x[3]), list(x[4])) for x in train_no_dups]

# Radomise
shuf_train = sample(train_clean_localised, k=len(train_clean_localised))
# Testing set
test_input_tokenised = [x[1] for x in test_trimmed]
test_output_tokenised = [x[3] for x in test_trimmed]
# Training set
train_input_tokenised = [x[1] for x in shuf_train]
