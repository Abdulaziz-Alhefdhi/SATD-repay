...
def create_tokenised(debt_lines, repaid_lines):
    debt_streams, repaid_streams = [], []
    for dl, rl in zip(debt_lines, repaid_lines):
        try:
            ds = javalang.tokenizer.tokenize(dl, ignore_errors=True)
            rs = javalang.tokenizer.tokenize(rl, ignore_errors=True)
            debt_streams.append(ds)
            repaid_streams.append(rs)
        except javalang.tokenizer.LexerError as le:
            sys.exit(le)
            continue
    debt_tokenised, repaid_tokenised = [], []
    for debt, repaid in zip(debt_streams, repaid_streams):
        # print(debt[0].value)
        debt_tokenised.append(" ".join([x.value for x in debt]))
        repaid_tokenised.append(" ".join([x.value for x in repaid]))

    return debt_tokenised, repaid_tokenised
...
def retrieve_data(data_dir):
    ...
    satdr_debt_tokenised, satdr_repaid_tokenised = create_tokenised(satdr_debt_no_spaces, satdr_repaid_no_spaces)

    print(len(satdr_debt_no_spaces), len(satdr_repaid_no_spaces))
    print(len(satdr_debt_tokenised), len(satdr_repaid_tokenised))
    sys.exit()
